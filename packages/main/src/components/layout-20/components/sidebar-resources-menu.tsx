...
import {
  ...
  AccordionMenuItem,
} from '@repo/main/components/ui/accordion-menu';
import { Badge } from '@repo/main/components/ui/badge';
import { usePathname } from 'next/navigation';
import Link from 'next/link';
import { MenuItem } from "./types";

export function SidebarResourcesMenu({MENU_SIDEBAR_RESOURCES}: { MENU_SIDEBAR_RESOURCES: MenuItem[] }) {
            ...
            {item.children?.map((child, index) => (
              <AccordionMenuItem key={index} value={child.path || '#'}>
                <Link href={child.path || '#'}>
                  ...
                  {child.badge  && <Badge size="sm" variant={child.badgeVariant || 'destructive'} appearance="light">{child.badge}</Badge>}
                  </Link>
              </AccordionMenuItem>
            ))}
          </AccordionMenuSubContent>
